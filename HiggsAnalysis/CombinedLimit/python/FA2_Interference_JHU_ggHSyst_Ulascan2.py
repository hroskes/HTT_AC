...
#        self.modelBuilder.factory_('expr::bsmCoupling_ggH("@0*@1**2", muf,a3_ggH)'.format(**xsecs))
        ...
        self.modelBuilder.factory_('expr::bsmCoupling_VBF("@0*@1**2 - @0*@1*@2*sqrt({sigma2_VBF}/{sigma1_VBF})", muV,a3,a1)'.format(**xsecs))
        self.modelBuilder.factory_('expr::bsmCoupling_ZH("@0*@1**2 - @0*@1*@2*sqrt({sigma2_ZH}/{sigma1_ZH})", muV,a3,a1)'.format(**xsecs))
        self.modelBuilder.factory_('expr::bsmCoupling_WH("@0*@1**2 - @0*@1*@2*sqrt({sigma2_WH}/{sigma1_WH})", muV,a3,a1)'.format(**xsecs))
        self.modelBuilder.factory_('expr::bsmCoupling_ggH("@0*@1**2", muf,a3_ggH)'.format(**xsecs))
...
#        self.modelBuilder.factory_('expr::intCoupling_WH("@0*@1*@2*sqrt({sigma2_WH}/{sigma1_WH})*2", muV,a1,a3)'.format(**xsecs))
    ...
    def getYieldScale(self,bin,process):
        ...
        if process in ["WH_htt_0PH",]:
            return 'bsmCoupling_WH'
        if process in ["ZH_htt_0PH",]:
            return 'bsmCoupling_ZH'
        if process in ["qqH_htt_0PHf05ph0"]:
            return 'intCoupling_VBF'
        if process in ["ZH_htt_0PHf05ph0"]:
            return 'intCoupling_ZH'
        if process in ["WH_htt_0PHf05ph0"]:
